...
public class TestIt {
    ...
    public void requestWithValidJobByBasic() {
        JobSeeker JOBSEEKER = new JobSeeker(new JobSeeker.Id(APPROVED_JOBSEEKER));
        HttpSession session = new HttpSession(JOBSEEKER);
    ...
    public void applyUsingExistingResume() {
        JobSeeker JOBSEEKER = new JobSeekerPremium(new JobSeeker.Id(JOBSEEKER_WITH_RESUME));
        HttpSession session = new HttpSession(JOBSEEKER);
    ...
    public void unapprovedBasic() {
        JobSeeker JOBSEEKER = new JobSeeker(new JobSeeker.Id(INCOMPLETE_JOBSEEKER));
        HttpSession session = new HttpSession(JOBSEEKER);
    ...
    private void addToJobseekerProfileRepository(int id, ProfileStatus status) {
        JobSeekerProfile profile = new JobSeekerProfile(new JobSeeker.Id(id), status);
    ...
    private void addJobToRepository(int jobId) {
        if (jobId != INVALID_JOB_ID) {
            jobRepository.addJob(new Job(new Job.Id(jobId)));
        }
    }
    ...
    private void setupController() {
        ...
        JobService jobService = new JobServiceImpl(jobApplicationSystem, resumeManager);
        JobPolicy jobPolicy = new PremiumCompletedProfilePolicy(jobseekerProfileManager);

        controller = new ApplyController(jobService, jobSearchService, resumeManager, jobPolicy);
    }
}
